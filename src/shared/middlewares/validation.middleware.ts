/**
 * Validation Middleware based on https://github.com/halvardssm/oak-middleware-validator
 * with changes only to make it work with current versions of Oak and Deno and to ensure
 * types compablity.
 */

import { Status, BodyType, FormDataReader } from 'oak';
...
export const validatorMiddleware = <T>(
  options: ValidatorMiddlewareOptions
): Middleware<T> => {
      ...
      switch (body.type) {
        ...
        default:
          if (!body.value && options.bodyRequired) {
            ...
            typeof valEl.validationOption === 'function' &&
            body.value
          ) {
            valEl.validationOption(
              valEl.key,
